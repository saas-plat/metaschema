...
const parseRule = exports.parseRule = (obj) => {
  if (isArray(obj)) {
    return obj.map(parseRule).reduce((ret, arr) =>  isArray(arr[0]) ? ret.concat(arr) : ret.concat([arr]), []);
    ...
    keys(obj).forEach(key => {
      if (key === 'or') {
        ...
        //debug(obj[key])
        let subconstraint = parseRule(obj[key]);
        if (subconstraint.length == 1 && isArray(subconstraint[0])) {
          subconstraint = subconstraint[0];
        }
        ...
        const constraint = [key];
        let subconstraint = parseRule(obj[key])[0];
        debug('subconstraint %o', subconstraint);
        if (!isArray(subconstraint)) {
        ...
        constraint.push(...subconstraint);
        moreconstraints.push(constraint);
      } else if (key === 'pattern' || key === 'condition') { // 自定义的key
...
exports.Rule = class Rule {
  ...
  static create(
    ...
    let whenArray = parseRule(when);
    return new Rule(name, whenArray, then, options);
  }
}
