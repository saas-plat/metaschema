...
exports.loadJson = (template, viewModelSchema) => {
...
const findModel = exports.findModel = () => null;

exports.View = class View {

  constructor(type, bind, props) {
    if (!type) {
      throw new Error(i18n.t('视图类型无效'));
    }

    const Model = findModel(type);
    if (!Model) {
      throw new Error(i18n.t('"{{type}}"模型不存在', {
        type
      }));
    }
    ...
    props = _mapValues(props, v => {
      if (_isArray(v)) {
        return v.map((v) => {
          ...
          type ? new View(type, bind, props) : v
        });
      } else if (v) {
        const {
          type,
        ...
        return type ? new View(type, bind, props) : v;
      } else {
        return v;
      }
    });
  ...
  static create(Template, template, viewModelSchema = {}) {
    ...
    return new View(type, bind, props);
  }
}
