...
const loadReduce = ({
  ...
  items = items && items.map(it => loadReduce(it,   keyGenerator)).filter(it => it);
...
const loadJson = exports.loadJson = (template,  keyGenerator) => {
  ...
  //   debug(ajv.errors);
  // }
  return loadReduce(template, keyGenerator);
}
...
exports.View = class View {
  ...
  // 从view中提取字段和行为作为vm的schema
  pickSchema(){

  }

  static create(name, Template, template) {
    const keyGenerator = {
      key: 1
    };
    const root = loadJson(mergeTemplate(Template, template) , keyGenerator);
