...
const parseFieldItem = (it, types) => {
  ...
  if (isString(it) || isSchemaType(it)) {
    ...
    type = getSchemaTypeKey(type) || type;
    ...
    if (type === 'function') {
      arguments1 = _arguments || it.arguments || [];
      if (isString(arguments1)) {
        arguments1 = arguments1.split(',').filter(it => it.trim());
      }
      handle = parseFunction(_handle || it.handle, false);
    }
    delete other.type;
    delete other.fields;
    ...
    if (rules.validator) {
      rules.validator = parseFunction(rules.validator, true);
      rules.arguments = rules.arguments || ['rule', 'value', 'source', 'options', 'context'];
      if (isString(rules.arguments)) {
        rules.arguments = rules.arguments.split(',').filter(it => it.trim());
      }
    } else {
      delete rules.arguments;
    }
    ...
    let sub;
    if (it.length > 0) {
      sub = parseFieldItem(it[0], types);
      if (!sub) {
        sub = {
          type: 'object',
          fields: parseFields(it[0], types)
        };
      }
    } else {
      sub = {
        type: 'object',
        fields: parseFields(it.fields, types)
      };
    }
    subtype = sub.type;
    subtype = getSchemaTypeKey(subtype) || subtype;
...
const parseFields = exports.parseFields = (obj, types) => {
  const fields = [];
  isPlainObject(obj) && keys(obj).forEach(key => {
    ...
    const field = parseFieldItem(obj[key], types);
    if (!field) {
...
const createObject = exports.createObject = (defineObj = {}, fields, opts) => {
  ...
  fields.forEach(it => {
    const customeCreate = exts[it.type];
    let created = false;
    if (customeCreate) {
      created = customeCreate(it, defineObj, opts);
    }
    if (!created) {
      switch (it.type) {
      case 'array':
...
const parseFunction = (handle, retValue = true) => {
  let fn;
  if (isArray(handle)) {
    if (handle.every(it => it.indexOf('return ') === -1) && retValue) {
      fn = handle.map((it, i) => i === handle.length - 1 ? 'return ' + it : it).join('\n');
    } else {
      fn = handle.join('\n');
    }
  } else if (isString(handle)) {
    if (handle.indexOf('return ') === -1 && retValue) {
      fn = 'return ' + handle;
    } else {
      fn = handle;
    }
...
const createFunction = exports.createFunction = (handle, args) => {
  if (isFunction(handle)) {
    debug('create function...', handle);
    return handle;
  } else if (isString(handle)) {
    debug('create function...', handle);
    return new Function(...args, handle);
  }
}

const createFields = exports.createFields = (fields) => {
  if (!fields) {
    return fields;
  }
  fields.forEach(it => {
    if (it.handle) {
      it.handle = createFunction(it.handle, it.arguments);
    } else if (it.rules && it.rules.validator) {
      it.rules.validator = createFunction(it.rules.validator, it.rules.arguments);
    }
  });
  return fields;
}
...
exports.Schema = class Schema {
  ...
  constructor({
    ...
    this.actions = createFields(actions);
  ...
  static create(BaseType, schema) {
    return new Schema(Schema.parse(BaseType, schema));
  }

  static parse(BaseType, schema) {
    const defFields = parseFields(BaseType.fields, BaseType.types);
    let fields = parseFields(schema, BaseType.types);
